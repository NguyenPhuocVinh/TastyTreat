...
export const checkoutController = {
    async createCheckout(req, res) {
        try {
            ...
            const checkPayment = await paymentServices.getPaymentByOrderId(
                orderId
            );
            if (checkPayment) {
                const payment =
                    await paymentServices.checkStatusPaymentByOrderId(orderId);
                if (payment === "PAID") {
                    return res
                        .status(StatusCodes.BAD_REQUEST)
                        .json({ message: "Order has been paid" });
                }
                if (payment === "PENDING") {
                    await paymentServices.updateVnpayRefByOrderId(
                        orderId,
                        vnp_TxnRef
                    );
                }
    ...
    async confirmCheckout(req, res) {
        try {
            ...
            if (secureHash === signed) {
                ...
                const orderId = await paymentServices.findPaymentByVnpayRef(
                    vnp_TxnRef
                );
                Promise.all([
                ...
                return res
                    ...
                    .json({ code: "00", message: "Payment successful" });
            } else {
                ...
                return res
                    .status(StatusCodes.BAD_REQUEST)
                    .json({ code: "97", message: "Invalid secure hash" });
            }
        } catch (error) {
